...
def _fits_smem(mnt: int, page_size: int, max_context_len: int) -> tuple[bool, int]:
    ...
    # NOTE: this estimate uses mnt as the seq-count proxy, but the real kernel
    # SMEM = num_seqs * pages_per_seq (see kernel get_smem_estimate_bytes). For
    # prefill (few long seqs) mnt grossly overestimates -> it wrongly pruned all
    # SWA p/m candidates. Let the actual TPU run gate OOM instead of this proxy.
    return True, est
