    ...
    def __init__(
        ...
        if not is_sparse:
            ...
            if self.use_fused:
                ...
                # layout ``model.layers.N.mlp.experts.*``. FusedEPMoEV2 (Strix
                # double-buffer kernel) and FusedTPMoEV4 (TP-MoE) both share
                # FusedEPMoE's __init__/weight layout — only the class differs,
                # and the shared expert stays external for all (num_shared_experts=0).
...
class BailingMoeV3ForCausalLM(nnx.Module):
    ...
    def _post_load_weights(self):
        """Split kv_b_proj into absorbed w_uk/w_uv for MLA layers; guard KDA
        gates; reshape MoE weights to TP layout for FUSED_V4 layers.

        The FUSED_V4 reshape comes FIRST because it allocates a transient
        replicated copy (~1 GiB/weight) per MoE layer and we want that headroom
        before MLA's own post-load processing kicks in.
        """
        # FUSED_V4: one-time EP -> TP reshard per MoE layer. No-op for other
        # backends because isinstance(...) is False there.
    ...
    def _add_moe_mappings(
        ...
        moe_backend_value = (
            moe_backend.value if isinstance(moe_backend, MoEBackend) else moe_backend
        )

        metadata = get_global_expert_location_metadata()
        phy_to_log = None
        if metadata is not None:
            # FUSED_V4 holds every expert on every chip — EPLB's physical->logical
            # remap has no meaning and would silently mis-route. The wrapper
            # already guards in __init__; fail here too so the mapping layer
            # can't quietly thread phy_to_log through.
            if moe_backend_value == "fused_v4":
                raise RuntimeError(
                    "MoEBackend.FUSED_V4 is incompatible with EPLB "
                    "(get_global_expert_location_metadata() returned a non-None "
                    "metadata object). Disable EPLB before selecting fused_v4."
                )
