...
def _fused_ep_moe_kernel(
    ...
    gather_recv_seen_x2_smem,  # (2, padded_num_experts)
    # --- VMEM scratch ---
    ...
    *,
    ...
    n_ffn1_chunk_sg = ffn1_chunk // quant_block_k if quant_block_k is not None else 1
    w1_scale_load_enabled = w1_scale_hbm is not None and not disable_w1_scale_load
    w3_scale_load_enabled = w3_scale_hbm is not None and not disable_w3_scale_load
    w2_scale_load_enabled = w2_scale_hbm is not None and not disable_w2_scale_load
    w1_scale_apply_enabled = w1_scale_load_enabled and not disable_w1_scale_apply
    w3_scale_apply_enabled = w3_scale_load_enabled and not disable_w3_scale_apply
    w2_scale_apply_enabled = w2_scale_load_enabled and not disable_w2_scale_apply
    ...
    def sync_barrier():
        if disable_sync_barrier:
            return
        with jax.named_scope("sync/barrier"):
    ...
    def start_fetch_topk(*, bt_id, priority=0):
        ...
        bt_start = bt_id * bt
        with jax.named_scope("topk/fetch"):
    ...
    def wait_fetch_topk(*, bt_id):
        bt_sem_id = bt_bank_id(bt_id)
        with jax.named_scope("topk/wait"):
            pltpu.make_async_copy(
    ...
    def all_reduce_metadata(*, bt_id, bt_sem_id, t2e_routing):
        ...
        def _inkernel_allreduce(
            ...
            if metadata_mode == "direct":
                ...
                # barriers between). Same total bytes / device, just different
                # schedule; mathematically identical d2e_count_vmem after.
    ...
    def start_a2a_scatter_batch(*, bt_sem_id, bt_start, a2a_bank_id):
        with jax.named_scope("a2a/scatter/start"):
    ...
    def wait_a2a_scatter_send_batch(*, a2a_bank_id):
        ...
        with jax.named_scope("a2a/scatter/send_wait"):
    ...
    def wait_a2a_scatter_recv(*, bt_sem_id, e_sem_id, local_e_id, a2a_bank_id):
        ...
        with jax.named_scope("a2a/scatter/recv_wait"):
    ...
    def start_a2a_gather(*, bt_sem_id, e_sem_id, local_e_id, a2a_bank_id, gather_bank_id):
        if disable_a2a or disable_a2a_gather:
            return
        with jax.named_scope("a2a/gather/start"):
    ...
    def wait_a2a_gather_send(*, bt_sem_id, e_sem_id, local_e_id, a2a_bank_id, gather_bank_id):
        ...
        with jax.named_scope("a2a/gather/send_wait"):
    ...
    def wait_a2a_gather_recv_all(*, bt_sem_id, gather_bank_id):
        ...
        def _wait_one_active_route(route_id, _):
            t_id = route_id // top_k
            k_id = route_id - t_id * top_k
            e_id = t2e_routing_x2_smem[bt_sem_id, t_id, k_id]
            is_valid = e_id >= 0
            e_id_safe = lax.select(is_valid, e_id, jnp.int32(0))
            seen = gather_recv_seen_x2_smem[bt_sem_id, e_id_safe]
            should_wait = jnp.logical_and(is_valid, seen == 0)

            @pl.when(should_wait)
            def _():
                gather_recv_seen_x2_smem[bt_sem_id, e_id_safe] = jnp.int32(1)
                _wait_one_gather_recv(e_id_safe)

            return None

        with jax.named_scope("a2a/gather/recv_wait"):

            if wait_gather_recv_active_only:

                def _clear_seen(i, _):
                    gather_recv_seen_x2_smem[bt_sem_id, i] = jnp.int32(0)
                    return None

                lax.fori_loop(0, num_experts, _clear_seen, None, unroll=False)
                lax.fori_loop(0, bt * top_k, _wait_one_active_route, None, unroll=False)
            else:
    ...
    def start_fetch_w1(local_e_id, slot, bf_id, priority=1):
        if disable_weight_load or disable_w1_load:
            return
        with jax.named_scope("expert/weights/w1/start"):
    ...
    def wait_fetch_w1(slot):
        if disable_weight_load or disable_w1_load:
            return
        with jax.named_scope("expert/weights/w1/wait"):
            pltpu.make_async_copy(
    ...
    def start_fetch_w3(local_e_id, slot, bf_id, priority=1):
        if disable_weight_load or disable_w3_load:
            return
        with jax.named_scope("expert/weights/w3/start"):
            for p in range(t_packing):
                pltpu.make_async_copy(
                    src_ref=w3_hbm.at[
                        local_e_id,
    ...
    def wait_fetch_w3(slot):
        if disable_weight_load or disable_w3_load:
            return
        with jax.named_scope("expert/weights/w3/wait"):
            pltpu.make_async_copy(
    ...
    def start_fetch_w2(local_e_id, slot, bf_id, priority=0):
        if disable_weight_load or disable_w2_load:
            return
        with jax.named_scope("expert/weights/w2/start"):
            for p in range(t_packing):
                pltpu.make_async_copy(
                    src_ref=w2_hbm.at[
                        local_e_id,
    ...
    def wait_fetch_w2(slot):
        if disable_weight_load or disable_w2_load:
            return
        with jax.named_scope("expert/weights/w2/wait"):
            pltpu.make_async_copy(
    ...
    def dequant_w1(slot):
        ...
        for p in range(t_packing):
            ...
            def _dq_w1(sg_id, _):
                ...
                if w1_scale_apply_enabled:
                    s = b_w1_scale_x2_vmem[slot, p, pl.ds(sg_id, 1), 0, pl.ds(0, bf)]
                    s = s.reshape(1, bf)
                    w_f32 = w_f32 * jnp.broadcast_to(s, (quant_block_k, bf))
        ...
        for p in range(t_packing):
            ...
            def _dq_w1(sg_id, _):
                ...
                if w1_scale_apply_enabled:
                    ...
                    w_f32 = w_f32 * jnp.broadcast_to(s, (quant_block_k, ffn1_chunk))
                w_bf16 = w_f32.astype(jnp.bfloat16)
                b_w1_dq_vmem.at[
                    p,
                    pl.ds(sg_off, quant_block_k),
            ...
            def bts_body(bts_id, next_bf0_prefetched):
                ...
                for bf_id in range(num_bf):
                    ...
                    if direct_scaled_dot_ffn1 and w1_scale_hbm is not None and bt <= 16:
                                ...
                                for p_id in range(t_packing):
                                    ...
                                    def _ffn1_gate_sg(sg_id, gate_acc, _pid=p_id):
                                        ...
                                        if w1_scale_apply_enabled:
                                            ...
                                            d1 = jnp.stack([d1[i] * s1 for i in range(btc)], axis=0)
                                        return gate_acc + d1
                                ...
                                for p_id in range(t_packing):
                                    ...
                                    def _ffn1_up_sg(sg_id, up_acc, _pid=p_id):
                                        ...
                                        if w3_scale_apply_enabled:
                                            ...
                                            d3 = jnp.stack([d3[i] * s3 for i in range(btc)], axis=0)
                                        return up_acc + d3
                        ...
                        with jax.named_scope("expert/ffn1/gate_up"):
                    ...
                    if (
                        ...
                        start_fetch_w13_w2(local_e_id, slot, next_bf_id, include_w2=False)
                                ...
                                if use_direct_w2:
                                    ...
                                    def _ffn2_sg_body(sg_id, partial_acc):
                                        ...
                                        if w2_scale_apply_enabled:
                                            ...
                                            d = jnp.stack([d[i] * s for i in range(btc)], axis=0)
                    ...
                    if not ffn1_stream_chunked_down:
                        with jax.named_scope("expert/ffn2/down"):
    ...
    def acc_and_store_output(*, bt_id, bt_sem_id, out_buf_id, gather_bank_id):
        ...
        def start_load_acc_bt(*, tile_start, buf_id):
            ...
            with jax.named_scope("output/acc/load"):
        ...
        def wait_load_acc_bt(*, buf_id, tile_start):
            ...
            def _():
                ...
                with jax.named_scope("output/acc/wait"):
        ...
        def acc_gather_to_output(*, tile_start, out_offset, buf_id):
            with jax.named_scope("output/acc/compute_store"):
                output_tile = jnp.zeros((acc_bt, t_packing, h_per_t), dtype=jnp.float32)
                if not disable_acc_and_store and not disable_acc_compute:
    ...
    def start_send_bo(*, bt_id, priority=0):
        ...
        bt_sem_id = bt_bank_id(bt_id)
        bt_start = bt_id * bt
        with jax.named_scope("output/send"):
            pltpu.make_async_copy(
    ...
    def run_bt(bt_id, e_sem_id, *, skip_post_gather=False):
        ...
        def prepare_bt_metadata(_bt_id, _bt_sem_id):
            ...
            with jax.named_scope("metadata/direct"):
                all_reduce_metadata(
                    bt_id=_bt_id,
        ...
        if not skip_post_gather and not disable_post_gather_path:
            ...
            if (
                ...
                with jax.named_scope("output/post_sync"):
                    sync_barrier()
    ...
    if not disable_kernel_start_sync:
        with jax.named_scope("kernel/start_sync"):
            sync_barrier()
...
@functools.partial(
    ...
    static_argnames=[
        ...
        "disable_w3_load",
        "disable_w2_load",
        "disable_w1_scale_load",
        "disable_w3_scale_load",
        "disable_w2_scale_load",
        "disable_w1_scale_apply",
...
def fused_ep_moe_v2(
    ...
    *,
    ...
    if disable_w1_scale_load or disable_w3_scale_load or disable_w2_scale_load:
        scope_name += (
            f"-no_scale_load_{int(disable_w1_scale_load)}"
            f"{int(disable_w3_scale_load)}{int(disable_w2_scale_load)}"
        )
    if disable_w1_scale_apply or disable_w3_scale_apply or disable_w2_scale_apply:
        scope_name += (
            f"-no_scale_apply_{int(disable_w1_scale_apply)}"
            f"{int(disable_w3_scale_apply)}{int(disable_w2_scale_apply)}"
        )
    ...
    if wait_gather_recv_active_only:
        scope_name += "-wait_gather_recv_active_only"
    ...
    scratch_shapes = (
        ...
        pltpu.SMEM((smem_banks, padded_num_experts), jnp.int32),  # gather_recv_seen
        ...
        (
            None
            if w1_scale is None
            else pltpu.VMEM((wb_slots, t_packing, h_per_t // quant_block_k, 1, bf), jnp.float32)
        ),  # W1 scale
        (
            None
            if w3_scale is None
        ...
        (
            None
            if w2_scale is None
            else pltpu.VMEM((wb_slots, t_packing, bf // quant_block_k, 1, h_per_t), jnp.float32)
        ),  # W2 scale
        ...
        (
            ...
            else pltpu.VMEM((t_packing, bf, h_per_t), jnp.bfloat16)
        ),  # W2 dequant
