...
def _prequant_tokens_kernel(x_ref, out_ref, *, hidden_size, h_per_t):
    tile = x_ref.shape[0]
    x_f32 = x_ref[...].reshape(tile, hidden_size).astype(jnp.float32)
    amax = jnp.max(jnp.abs(x_f32), axis=-1, keepdims=True)
    scale = jnp.maximum(amax / jnp.float32(FP8_E4M3_MAX), jnp.float32(1e-12))
    x_fp8 = (x_f32 / scale).astype(jnp.float8_e4m3fn)
    scale_i32 = lax.bitcast_convert_type(scale.squeeze(-1), jnp.int32)
    b0 = lax.bitcast_convert_type(
        (scale_i32 & jnp.int32(0xFF)).astype(jnp.int8), jnp.float8_e4m3fn)
    ...
    b3 = lax.bitcast_convert_type(
        ...
        jnp.float8_e4m3fn)
    x_pre = x_fp8[:, :h_per_t - 4]
    scale_bytes = jnp.concatenate(
        [b0[:, None], b1[:, None], b2[:, None], b3[:, None]], axis=1)
    x_post = x_fp8[:, h_per_t:]
    x_fp8 = jnp.concatenate([x_pre, scale_bytes, x_post], axis=1)
    t_packing = hidden_size // h_per_t
    out_ref[...] = x_fp8.reshape(tile, t_packing, h_per_t)
...
def _fused_ep_moe_kernel(
    ...
    b_output_x2_vmem,        # (2, bt, hidden_size) out_dtype
                    ...
                    if direct_scaled_dot_ffn1 and w1_scale_hbm is not None:
                        ...
                        def _gate_only_btc(btc_id, ___):
                            ...
                            if not disable_dynamic_ffn1:
                                if dynamic_activation_quant:
                                    x_s = b_x_scale_vmem[pl.ds(btc_id * btc, btc), 0:1]
                    ...
                    if not disable_expert_stage_writeback:
                        def writeback_btc(btc_id, ___):
                            if t_packing == out_packing:
                                ...
                                for out_p in range(out_packing):
                                    lo = b_y_acc_vmem[
                                        pl.ds(btc_id * btc, btc), out_p * 2, pl.ds(0, h_per_t)
                                    ]
                                    hi = b_y_acc_vmem[
                                        pl.ds(btc_id * btc, btc), out_p * 2 + 1, pl.ds(0, h_per_t)
                                    ]
                                    b_y_stage_vmem.at[
                                        pl.ds(btc_id * btc, btc), out_p, pl.ds(0, h_per_t)
                                    ][...] = lo.astype(out_dtype)
                                    b_y_stage_vmem.at[
                                        pl.ds(btc_id * btc, btc), out_p, pl.ds(h_per_t, h_per_t)
                                    ][...] = hi.astype(out_dtype)
                            return None
...
def fused_ep_moe_v2(
    ...
    *,
    ...
    t_dtype = jnp.float8_e4m3fn if dynamic_activation_quant else tokens.dtype
    t_packing = get_dtype_packing(t_dtype)
    h_per_t = hidden_size // t_packing
        ...
        prequant_call = pl.pallas_call(
            ...
            grid_spec=pltpu.PrefetchScalarGridSpec(
                ...
                out_specs=pl.BlockSpec(
                    (bt, t_packing, h_per_t),
                    lambda i: (i, 0, 0),
                ),
            ),
            name="prequant-tokens",
        )
    else:
        prequant_call = None
    ...
    scratch_shapes = (
        ...
        # VMEM: gather accumulation (out_dtype for output path)
        ...
        # VMEM: output double buffer (out_dtype)
        ...
        # VMEM: token staging per bts tile (t_dtype — fp8 when pre-quantized)
    ...
    def kernel(
        ...
        if prequant_call is not None:
            tokens = prequant_call(tokens)
