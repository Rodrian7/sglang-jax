...
class Scheduler(
    ...
    def handle_generate_request(
        ...
        # PD disaggregation routing keys (Stage 2). Passed through from
        # the HTTP body via TokenizedGenerateReqInput; the decode-side
        # Mixin uses bootstrap_room to look up the prefill peer.
...
def run_scheduler_process(
    ...
    try:
        ...
        # PD disaggregation wiring (Stage 2). Engine entry installs
        # the manager / notifier / queues based on
        # ServerArgs.disaggregation_mode. The Mixin event-loop
        # methods rely on these attributes.
...
def _install_disaggregation_wiring(scheduler, server_args) -> None:
    ...
    Stage 2 review C2: spawn a heartbeat daemon for prefill so the
    bootstrap registration doesn't fall off the registry after the
    30s TTL.

    Stage 3 multi-host: derive the per-host advertised IP via
    :func:`resolve_host_ip` so multi-host deployments register the
    correct DCN-reachable address. Operators can pass
    ``--disaggregation-host-ip`` to override; otherwise the helper
    walks ``$HOSTNAME`` then ``socket.gethostname()``.
    """
    ...
    # Stage 4 H-C: resolve the effective shared secret once and pass
    # it to every channel constructor so they all agree.
    ...
    if scheduler.disagg_use_d2h_staging:
        # Fail loudly at boot, not at first prefill request. Stage 4
        # H-E review C2: model-specific QueueHostKVPool wiring is a
        # follow-up; until it lands, --disaggregation-enable-d2h must
        # be left at its default (False).
        raise RuntimeError(
            "--disaggregation-enable-d2h=true requires a wired "
            "QueueHostKVPool on the JaxTransferKVManager. The "
            "Stage 4 scheduler integration currently passes "
            "host_pool=None, so producer_handoff() would crash on "
            "the first prefill request. Run with the default "
            "(--no-disaggregation-enable-d2h) until path A is "
            "plumbed; see pd_runbook.md §8."
        )
    ...
    # Stage 4 H-D: install a SIGTERM handler that runs the documented
    # shutdown sequence (unregister → drain → stop reaper → stop
    # notifier). Best-effort; this is only installed if we are on the
    # main thread (signal.signal would otherwise raise). The
    # in-process scheduler thread can't install handlers, so the
    # caller is expected to forward SIGTERM via scheduler.shutdown().
...
def _make_disagg_shutdown(scheduler, mode: str):
    """Closure that runs the documented Stage 4 H-D shutdown order:
